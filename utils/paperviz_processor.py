...
class PaperVizProcessor:
    ...
    async def process_single_query(
        ...
        if exp_mode == "vanilla":
            print(f"[DEBUG] Starting Vanilla Agent processing...")
            ...
            print(f"[DEBUG] Starting Retriever Agent processing...")
            data = await self.retriever_agent.process(data, retrieval_setting=retrieval_setting)
            print(f"[DEBUG] Starting Planner Agent processing...")
            data = await self.planner_agent.process(data)
            print(f"[DEBUG] Starting Visualizer Agent processing...")
            data = await self.visualizer_agent.process(data)
            ...
            print(f"[DEBUG] Starting Planner Agent processing...")
            data = await self.planner_agent.process(data)
            print(f"[DEBUG] Starting Stylist Agent processing...")
            ...
            print(f"[DEBUG] Starting Critic Agent iterations (max {max_rounds})...")
            ...
            print(f"[DEBUG] [dev_full] Starting Retriever Agent processing...")
            data = await self.retriever_agent.process(data, retrieval_setting=retrieval_setting)
            print(f"[DEBUG] [dev_full] Starting Planner Agent processing...")
            data = await self.planner_agent.process(data)
            print(f"[DEBUG] [dev_full] Starting Stylist Agent processing...")
            data = await self.stylist_agent.process(data)
            print(f"[DEBUG] [dev_full] Starting Visualizer Agent processing...")
            data = await self.visualizer_agent.process(data)
            # Use max_critic_rounds from data (if set) or config
            max_rounds = data.get("max_critic_rounds", self.exp_config.max_critic_rounds)
            print(f"[DEBUG] [dev_full] Starting Critic Agent iterations (max {max_rounds})...")
            data = await self._run_critic_iterations(data, task_name, max_rounds=max_rounds, source="stylist")
            if "demo" in exp_mode: do_eval = False
        
        elif exp_mode == "dev_polish":
            print(f"[DEBUG] Starting Polish Agent processing...")
            ...
            print(f"[DEBUG] Starting Retriever Agent processing (solo)...")
    ...
    async def process_queries_batch(
        ...
        async def process_with_semaphore(doc):
            print(f"[DEBUG] -> Acquring semaphore for candidate {doc.get('candidate_id', 'N/A')}", flush=True)
            async with semaphore:
                print(f"[DEBUG] -> Semaphore acquired for candidate {doc.get('candidate_id', 'N/A')}", flush=True)
